...
# calculate relatedness score
  ...
  ggsave(glue("./database/{species}- Relatedness_vs_Avg_Litters_per_Month.jpeg"), plot = p_litters_per_month, width = 8, height = 6, device = "jpeg")
  ...
  # Plot 6: Relatedness vs Average Litter Size (Weaning)
  ...
  merged_data_with_relatedness <- merged_data %>%
    ...
    filter(Relatedness > 0 & interval_days > 22 & interval_days < 365)
  ...
  delivery_intervals <- data %>%  left_join(DAMSIRE, by = c("Mating Number" = "MatingNumber")) %>%
    ...
    filter (Relatedness >0 & delivery_interval_days > 22 & delivery_interval_days < 365 )
 ...
 ########################## Relatedness groups
  ...
  print(p_litter)
  ggsave("./database/Loss_Ratio_Litter_Relatedness_Comparison.jpeg", plot = p_litter, width = 5, height = 8, device = "jpeg")
  ...
  print(p_mice)
  ggsave("./database/Loss_Ratio_Mice_Relatedness_Comparison.jpeg", plot = p_mice, width = 5, height = 8, device = "jpeg")
  ...
  ########################################
  meaning_combined_results_chi <- combined_results_chi %>%
    filter(Relatedness >0 & num_litters > 4 ) #%>%
    #arrange(significance_direction,  chi_squared_test_value) #desc(Relatedness)
  
  ##########################
  # Calculate correlations for each direction of loss
  cor_higher <- cor.test(meaning_combined_results_chi$Relatedness[meaning_combined_results_chi$significance_direction == "Higher than Expected"],
                         meaning_combined_results_chi$test_p_value[meaning_combined_results_chi$significance_direction == "Higher than Expected"])
  
  cor_lower <- cor.test(meaning_combined_results_chi$Relatedness[meaning_combined_results_chi$significance_direction == "Lower than Expected"],
                        meaning_combined_results_chi$test_p_value[meaning_combined_results_chi$significance_direction == "Lower than Expected"])
  
  
  # Plot with ggplot2 to visualize the direction of loss with separate correlation labels
  p <- ggplot(meaning_combined_results_chi, aes(x = Relatedness, y = test_p_value, color = significance_direction)) +
    geom_point(size = 3) +                                             # Scatter plot with color-coded points
    geom_smooth(method = "lm", se = FALSE) +                           # Line of best fit for each group
    annotate("text", x = 0.15, y = 1.1, label = paste0("Higher (R = ", round(cor_higher$estimate, 4), ", p = ", format.pval(cor_higher$p.value, digits = 4), ")"),
             color = "blue", hjust = 0) +                              # Add annotation for Higher than Expected
    annotate("text", x = 50, y = 1.1, label = paste0("Lower (R = ", round(cor_lower$estimate, 4), ", p = ", format.pval(cor_lower$p.value, digits = 4), ")"),
             color = "red", hjust = 0) +                             # Add annotation for Lower than Expected
    scale_color_manual(values = c("Higher than Expected" = "blue", 
                                  "Lower than Expected" = "red", 
                                  "Equal to Expected" = "grey")) +     # Custom colors for significance direction
    labs(title = "Correlation between Relatedness and Chi-Squared Test Value with Direction of Loss",
         x = "Relatedness",
         y = "Chi-Squared Test Value",
         color = "Direction of Loss") +
    theme_minimal() +
  ...
  ggsave(glue("./database/{species}-Correlation between Relatedness and Chi-Squared Test Value with Direction of Loss.jpeg"))
  
  
  # # Define significance threshold (e.g., p < 0.05 is significant)
  # threshold <- 0.05
  # 
  # # Categorize the data into 'Significant' and 'Insignificant' based on chi_squared_test_value
  # meaning_combined_results_chi <- meaning_combined_results_chi %>%
  #   mutate(significance = ifelse(test_p_value < threshold, "Significant", "Insignificant"))
  # 
  # # Plot the Relatedness for Significant and Insignificant categories
  # p2 <- ggplot(meaning_combined_results_chi, aes(x = significance, y = Relatedness)) +
  #   geom_boxplot(fill = "lightblue", color = "black") +  # Boxplot to show distribution
  #   geom_jitter(width = 0.2, color = "blue") +           # Add points for individual data
  #   labs(title = "Relatedness for Significant and Insignificant Chi-Squared Tests",
  #        x = "Chi-Squared Test Result",
  #        y = "Relatedness") +
  #   theme_minimal()
  # 
  # # Print the plot
  ...
  combined_results_chi_binomal <- results_chi_square_binomial %>%
  left_join(results_df %>% dplyr:: select(MatingNumber, Relatedness), by = c(`Mating Number` = "MatingNumber")) %>%
    left_join(mice_summary, by = "Mating Number") %>%  # Join mice summary
    mutate(
      loss_ratio_litter = num_losses / num_litters,            # Loss ratio based on litters
  ...
  #write.xlsx( combined_results_chi_binomal, glue("./database/mice loss at weaning - {species}- chiquare_bionomal.xlsx"))
  
  ##################
  combined_results_chi_binomal <- combined_results_chi_binomal %>%
    filter(Relatedness >0 & num_litters > 4 ) #%>%
  #arrange(significance_direction,  chi_squared_test_value) #desc(Relatedness)
  ...
  # Calculate correlations for each direction of loss
  cor_higher_chi_bino <- cor.test(combined_results_chi_binomal$Relatedness[meaning_combined_results_chi$significance_direction == "Higher than Expected"],
                         combined_results_chi_binomal$test_p_value[meaning_combined_results_chi$significance_direction == "Higher than Expected"])
  
  cor_lower_chi_bino <- cor.test(combined_results_chi_binomal$Relatedness[meaning_combined_results_chi$significance_direction == "Lower than Expected"],
                        combined_results_chi_binomal$test_p_value[meaning_combined_results_chi$significance_direction == "Lower than Expected"])
  
  
  # Plot with ggplot2 to visualize the direction of loss with separate correlation labels
  p_chi_bino <- ggplot(combined_results_chi_binomal, aes(x = Relatedness, y = test_p_value, color = significance_direction)) +
    geom_point(size = 3) +                                             # Scatter plot with color-coded points
    geom_smooth(method = "lm", se = FALSE) +                           # Line of best fit for each group
    annotate("text", x = 0.15, y = 1.1, label = paste0("Higher (R = ", round(cor_higher_chi_bino$estimate, 4), ", p = ", format.pval(cor_higher_chi_bino$p.value, digits = 4), ")"),
             color = "blue", hjust = 0) +                              # Add annotation for Higher than Expected
    annotate("text", x = 50, y = 1.1, label = paste0("Lower (R = ", round(cor_lower_chi_bino$estimate, 4), ", p = ", format.pval(cor_lower_chi_bino$p.value, digits = 4), ")"),
             color = "red", hjust = 0) +                             # Add annotation for Lower than Expected
    scale_color_manual(values = c("Higher than Expected" = "blue", 
                                  "Lower than Expected" = "red", 
                                  "Equal to Expected" = "grey")) +     # Custom colors for significance direction
    labs(title = "Correlation between Relatedness and p Value with Direction of Loss (chi-square and binomal tests)",
         x = "Relatedness",
         y = "Chi-Squared Test Value",
         color = "Direction of Loss") +
    theme_minimal() +
  ...
  ggsave(glue("./database/{species}- Correlation between Relatedness and p Value with Direction of Loss (chi-square and binomal tests).jpeg"))
